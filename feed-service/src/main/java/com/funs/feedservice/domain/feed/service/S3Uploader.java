...
import com.funs.feedservice.domain.feed.exception.NotConvertFileException;
...
public class S3Uploader {
    ...
    public String upload(Long gifticonId, MultipartFile multipartFile, String dirName) throws IOException {
        ...
        System.out.println("피드 service에 필요한 s3Uploader convert 호출 직후");
    ...
    // s3로 파일 업로드하기
    private String upload(Long feedId, File uploadFile, String dirName) {
        String fileName = dirName + "/" + UUID.randomUUID() + uploadFile.getName();   // S3에 저장된 파일 이름
        String uploadImageUrl = putS3(uploadFile, fileName); // s3로 업로드
        System.out.println("기프티콘 service에 필요한 s3Uploader putS3 사진 파일 업로드 성공");

//        Feed feed = feedRepository.findById(feedId)
//                        .orElseThrow(() -> new NotExistFeedException("해당 피드가 존재하지 않습니다."));
//
//        feed.setImageUrl(uploadImageUrl);
...
//        removeNewFile(uploadFile);
    ...
    private Optional<File> convert(MultipartFile file) throws IOException {
        ...
        System.out.println("피드 service에 필요한 s3Uploader convert 호출 성공");
